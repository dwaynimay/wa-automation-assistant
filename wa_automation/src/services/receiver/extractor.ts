...
export async function extractMessageData(msg: any): Promise<MessageData> {
  ...
  let idPesan = "unknown", waktu = 0, fromMe = false;
  let idChat = "unknown", isGroup = false, pengirimAsli = "unknown";
  let namaProfil = "Orang", namaKontak = "", namaPanggilan = "Orang";
  let tipePesan = "unknown", teks = "";
  let isReply = false, idPesanDibalas = undefined, teksDibalas = undefined;
  let hasMedia = false, mimeType = undefined;
  ...
  } catch (e) { console.error("   [Extractor] ERROR di Cekpoint 1:", e); }

  // CEKPOINT 2: Routing & ID
  try {
    idChat = typeof msg?.from === 'string' ? msg.from : (msg?.from?._serialized || String(msg?.from || "unknown"));
    isGroup = msg?.isGroup || false;
    pengirimAsli = isGroup && msg?.author ? msg.author : idChat;
    console.log("   [Extractor] Cekpoint 2 (Routing): AMAN");
  } catch (e) { console.error("   [Extractor] ERROR di Cekpoint 2:", e); }
  ...
  } catch (e) { console.error("   [Extractor] ERROR di Cekpoint 3:", e); }

  // CEKPOINT 4: Ekstraksi Teks
  try {
    tipePesan = msg?.type || 'unknown';
    const teksMentah = msg?.body || msg?.caption || msg?.content || "";
    teks = typeof teksMentah === 'string' ? teksMentah.trim() : String(teksMentah).trim();
    console.log("   [Extractor] Cekpoint 4 (Teks): AMAN");
  } catch (e) { console.error("   [Extractor] ERROR di Cekpoint 4:", e); }
  ...
  try {
    ...
    if (isReply && idPesanDibalas) {
      ...
      const tMentah = pesanAsli?.body || pesanAsli?.caption || msg?.quotedMsgObj?.body || "";
      teksDibalas = typeof tMentah === 'string' ? tMentah.trim() : String(tMentah).trim();
    }
    console.log("   [Extractor] Cekpoint 5 (Reply): AMAN");
  } catch (e) { console.error("   [Extractor] ERROR di Cekpoint 5:", e); }
  ...
  } catch (e) { console.error("   [Extractor] ERROR di Cekpoint 6:", e); }

  console.log("   [Extractor] SEMUA CEKPOINT SELESAI. Menyerahkan data kembali ke Processor.");

  return {
    idPesan, waktu, fromMe, idChat, isGroup, pengirimAsli,
    namaProfil, namaKontak, namaPanggilan, tipePesan, teks,
    isReply, idPesanDibalas, teksDibalas, hasMedia, mimeType
  };
}